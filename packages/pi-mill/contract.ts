...
export const SubagentSchema = Type.Object({
  ...
  code: Type.String({
    description:
      "TypeScript script using pi-mill's extension-specific `mill` global. Use mill.task({ agent: roleLabel, model, system, prompt }) to create task actors, call .start(), then await task.done. The script runs as a top-level module — use await and Promise.all directly.",
  }),
});
