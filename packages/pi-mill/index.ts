...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "Write a TypeScript script. `mill` is a global (like `process` or `console`). Use mill.task() to orchestrate agents.",
      "mill.task() returns a Promise<ExecutionResult>. Use `await` for sequential, `Promise.all` for parallel.",
      "Each task needs: agent, systemPrompt, prompt, model. cwd defaults to process.cwd().",
      ...
      "Available types: Mill, ExecutionResult, SubagentTaskInput, UsageStats.",
