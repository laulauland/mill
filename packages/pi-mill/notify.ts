...
const CUSTOM_TYPE = "pi-mill:complete";
...
export function registerMessageRenderer(pi: ExtensionAPI): void {
  pi.registerMessageRenderer(CUSTOM_TYPE, (message, { expanded }, theme) => {
    ...
    for (const summary of summaries) {
      ...
      const elapsed = formatElapsed(elapsedMs(summary));
      const color =
        summary.status === "done" ? "success" : summary.status === "failed" ? "error" : "warning";

      lines.push(
        `${theme.fg(color, icon)} ${theme.fg("accent", name)}  ${summary.status}  ${theme.fg("dim", elapsed)}`,
      );
      ...
      if (expanded) {
        ...
        if (resultText) {
          const truncated =
            resultText.length > TEXT_TRUNCATE
              ? resultText.slice(0, TEXT_TRUNCATE) + "…"
              : resultText;
